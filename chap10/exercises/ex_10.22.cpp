...
bool shortThan(const string &, std::size_t);
...
void biggies(vector<string> &words, vector<string>::size_type sz) {
    ...
    auto wc = std::stable_partition(words.begin(), words.end(), std::bind(shortThan, _1, sz));
    //计算满足 size() >= sz 的元素的数目
...
bool shortThan(const string &s, std::size_t sz) {
    return s.size() < sz;
}